...
async function fetchMenu() {
  try {
    ...
    // Convert menu object structure to flat array
    const menuObj = data.menu || data.items || data;
    if (typeof menuObj === 'object' && !Array.isArray(menuObj)) {
      // Flatten the menu structure (categories are keys)
      appState.menuItems = [];
    ...
    } else if (Array.isArray(menuObj)) {
      appState.menuItems = menuObj;
    } else {
...
// Menu Functions
function getCategoryMapping(tabCategory) {
  // Map tab names to backend category names
  const mapping = {
    'all': 'all',
  ...
  return mapping[tabCategory] || tabCategory;
}
...
function renderMenu(filter = 'all', searchQuery = '') {
  ...
  if (filter !== 'all') {
    const mappedCategory = getCategoryMapping(filter);
    filteredItems = filteredItems.filter(item => item.category === mappedCategory);
  }
